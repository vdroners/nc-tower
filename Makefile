...
.PHONY: build deploy ship gate-preflight sidecar-up sidecar-down bump-patch bump-minor test

# Upstream ships prebuilt js/; no npm package.json required for deploy.
build:
	@echo "Control Tower: prebuilt assets — nothing to compile"
	@test -f "$(ROOT)js/nc_tower-main.js" || (echo "missing js/nc_tower-main.js" && exit 1)
...
deploy: build
	...
	for dir in appinfo css img js lib templates l10n screenshots docs tools sidecar; do \
		if [ -d "$(ROOT)$$dir" ]; then \
