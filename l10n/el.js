OC.L10N.register(
    "nc_tower",
    {
    "app enabled": "η εφαρμογή είναι ενεργοποιημένη",
"could not enable app": "δεν μπόρεσε να ενεργοποιήσει την εφαρμογή",
"app disabled": "η εφαρμογή είναι απενεργοποιημένη",
"could not disable app": "δεν ήταν δυνατή η απενεργοποίηση της εφαρμογής",
"default quota": "προεπιλεγμένο όριο",
"unlimited": "απεριόριστος",
"added new group": "προστέθηκε νέα ομάδα",
"could not add new group - group already exists": "δεν ήταν δυνατή η προσθήκη νέας ομάδας - η ομάδα υπάρχει ήδη",
"Attention!!!\n": "Προσοχή!!!",
"This will delete the group": "Αυτό θα διαγράψει την ομάδα",
"this will delete user {name}": "Αυτό θα διαγράψει τον χρήστη {name}",
"group deleted": "η ομάδα διαγράφηκε",
"could not delete group": "δεν ήταν δυνατή η διαγραφή της ομάδας",
"action canceled": "η ενέργεια ακυρώθηκε",
"added new user": "προστέθηκε νέος χρήστης",
"all users": "όλους τους χρήστες",
"Could not edit user": "Δεν ήταν δυνατή η επεξεργασία του χρήστη",
"could not add new user - user already exists": "Δεν ήταν δυνατή η προσθήκη νέου χρήστη - ο χρήστης υπάρχει ήδη",
"App has been successfully updated": "Η εφαρμογή ενημερώθηκε με επιτυχία",
"Update response invalid": "Μη έγκυρη απάντηση ενημέρωσης",
"Network Error": "Σφάλμα δικτύου",
"show enabled": "εμφάνιση ενεργοποιημένη",
"show disabled": "εμφάνιση απενεργοποιημένο",
"Disable": "Καθιστώ ανίκανο",
"featured": "που παρουσιάζεται",
"Enable": "Καθιστώ ικανό",
"Overview": "Επισκόπηση",
"apps": "εφαρμογές",
"new user account": "νέο λογαριασμό χρήστη",
"hide groups": "απόκρυψη ομάδων",
"groups": "ομάδες",
"show groups": "εμφάνιση ομάδων",
"delete this group": "διαγράψτε αυτήν την ομάδα",
"new group": "νέα ομάδα",
"system": "σύστημα",
"apps with updates available": "εφαρμογές με διαθέσιμες ενημερώσεις",
"Personal settings": "Προσωπικές ρυθμίσεις",
"Administration settings": "Ρυθμίσεις διαχείρισης",
"edit user": "επεξεργασία χρήστη",
"Display name:": "Εμφανιζόμενο όνομα:",
"Password:": "Σύνθημα:",
"Email": "E-mail",
"Groups": "Ομάδες",
"Please select ...": "Επιλέξτε...",
"Group admin for": "Διαχειριστής ομάδας για",
"quota": "ποσοστό",
"Manager": "Διευθυντής",
"send": "στέλνω",
"app updates": "ενημερώσεις εφαρμογών",
"Available Updates": "Διαθέσιμες ενημερώσεις",
"update {name}": "ενημερώστε το {name}",
"All apps are up to date.": "Όλες οι εφαρμογές είναι ενημερωμένες.",
"User": "Μεταχειριζόμενος",
"data storage": "αποθήκευση δεδομένων",
"{value}% free": "{value}% δωρεάν",
"{value}% used": "Χρησιμοποιείται {value}%.",
"database": "βάση δεδομένων",
"typ": "τυπ",
"version": "εκδοχή",
"size": "μέγεθος",
"all apps": "όλες οι εφαρμογές",
"enabled": "ενεργοποιημένη",
"disabled": "ανάπηρος",
"hostname": "όνομα κεντρικού υπολογιστή",
"OS": "OS",
"processor": "επεξεργαστή",
"Nextcloud": "Nextcloud",
"current version": "τρέχουσα έκδοση",
"new version": "νέα έκδοση",
"datadirectory": "καταλόγου δεδομένων",
"update available": "διαθέσιμη ενημέρωση",
"no update available": "δεν υπάρχει διαθέσιμη ενημέρωση",
"CPU Usage": "Χρήση CPU",
"last 1 minute": "τελευταίο 1 λεπτό",
"last 5 minutes": "τελευταία 5 λεπτά",
"last 15 minutes": "τελευταία 15 λεπτά",
"RAM": "ΕΜΒΟΛΟ",
"{value} total": "Σύνολο {value}",
"{value} used": "Χρησιμοποιείται {value}",
"{value} free": "{value} δωρεάν",
"Storage space": "Αποθηκευτικός χώρος",
"Loading data or no disks found ...": "Φόρτωση δεδομένων ή δεν βρέθηκαν δίσκοι...",
"Loading data ...": "Φόρτωση δεδομένων…",
"PHP": "PHP",
"memory limit": "όριο μνήμης",
"max execution time": "μέγιστος χρόνος εκτέλεσης",
"max upload size": "μέγιστο μέγεθος μεταφόρτωσης",
"opcache freq": "opcache συχν",
"extensions": "επεκτάσεις",
"attention!": "Προσοχή!",
"this will send a notification": "αυτό θα στείλει μια ειδοποίηση",
"user deleted": "χρήστης διαγράφηκε",
"could not delete user": "δεν ήταν δυνατή η διαγραφή του χρήστη",
"You canceled!": "Ακυρώσατε!",
"first login": "πρώτη σύνδεση",
"never": "ποτέ",
"not available": "δεν είναι διαθέσιμο",
"invalid date value": "μη έγκυρη τιμή ημερομηνίας",
"last login": "τελευταία σύνδεση",
"administrator": "διαχειριστής",
"managed by": "διαχειρίζεται από",
"not managed yet": "δεν έχει καταφέρει ακόμα",
"send notification": "αποστολή ειδοποίησης",
"delete user": "διαγραφή χρήστη",
"user": "μεταχειριζόμενος",
"display name": "εμφανιζόμενο όνομα",
"password": "σύνθημα",
"email": "e-mail",
"please select ...": "παρακαλώ επιλέξτε...",
"group administrator for": "διαχειριστής ομάδας για",
"manager": "διευθυντής",
"Could not update app.": "Δεν ήταν δυνατή η ενημέρωση της εφαρμογής.",
"Threads": "Νήματα",
"message from %1$s": "μήνυμα από %1$s",
"message sent": "εστάλη μήνυμα",
"insert message ...": "εισαγωγή μηνύματος...",
"send message": "στείλετε μήνυμα",
"Only normal plain text with up to 4000 characters may be used. So no HTML code or other special formatting": "Επιτρέπεται η χρήση μόνο κανονικού απλού κειμένου με έως 4000 χαρακτήρες. Έτσι δεν υπάρχει κώδικας HTML ή άλλη ειδική μορφοποίηση",
"%1$s - behind reverse proxy": "%1$s - πίσω από τον αντίστροφο διακομιστή μεσολάβησης",
"%1$s - without reverse proxy": "%1$s - χωρίς αντίστροφο διακομιστή μεσολάβησης",
"unknown": "άγνωστος",
"type of installation": "τύπος εγκατάστασης",
"web server": "διακομιστή ιστού",
"virtual machine": "εικονική μηχανή",
"local installation": "τοπική εγκατάσταση",
"logfile": "αρχείο καταγραφής",
"network interfaces": "διεπαφές δικτύου",
"speed:": "ταχύτητα:",
"status:": "κατάσταση:",
"<strong>update channel:</strong> {name}": "<strong>ενημέρωση καναλιού:</strong> {name}",
"More information regarding the log file will be available as soon as our additional Nextcloud app LogCleaner is installed, which can be used to clean up the log file.": "Περισσότερες πληροφορίες σχετικά με το αρχείο καταγραφής θα είναι διαθέσιμες μόλις εγκατασταθεί η πρόσθετη εφαρμογή Nextcloud LogCleaner, η οποία μπορεί να χρησιμοποιηθεί για την εκκαθάριση του αρχείου καταγραφής.",
"Here is an example of what it would look like:": "Ακολουθεί ένα παράδειγμα για το πώς θα μοιάζει:",
"LogCleaner on GitHub": "LogCleaner στο GitHub",
"LogCleaner at the Nextcloud App store": "LogCleaner στο κατάστημα εφαρμογών Nextcloud",
"Update available": "Διαθέσιμη ενημέρωση",
"Updates available": "Διαθέσιμες ενημερώσεις",
"no Updates available": "δεν υπάρχουν διαθέσιμες ενημερώσεις",
"Nextcloud Update": "Nextcloud Ενημέρωση",
"no Nextcloud Update available": "δεν υπάρχει διαθέσιμη ενημέρωση Nextcloud",
"App Updates": "Ενημερώσεις εφαρμογών",
"no App Updates available": "δεν υπάρχουν διαθέσιμες ενημερώσεις εφαρμογής",
"Update": "Εκσυγχρονίζω",
"Update now": "Ενημέρωση τώρα",
"_%n App_::_%n Apps_": ["%n Εφαρμογή","%n Εφαρμογές"],
"Update Checker": "Ενημέρωση Checker"
},
"nplurals=2; plural=n != 1;");
